...
function UTCharaJpeg:init()
    ...
    self:registerAct("Tell Story", "", "all")
...
function UTCharaJpeg:onAct(battler, name)
    if name == "Check" then
        if self.check_count == 0 then
		    ...
		    return {
                "* "..self.name.." - i, your humble servant,[wait:5] wish for nothing but [color:red]genocides >=]"
            }
		else
		    return {
                "* "..string.upper(self.name).." - An iconic demon forgotten by time."
            }
		end

    elseif name == "lol" then
        ...
        return {
            "a",
            "Beware the [color:red]crimson attacks[color:reset].\n* They do not deal damage to your HP, but instead to your [color:yellow]maximum HP[color:reset]."
        }
